...
SYMBOLIC_NAME_RE = re.compile(r"([A-Z]|[a-z]|_)([0-9]|[A-Z]|[a-z]|_)*")
...
SBE_TYPES_TYPE = enum.Enum(
    "SBE_TYPES_TYPE",
    {"TYPE": "type", "COMPOSITE": "composite", "ENUM": "enum", "SET": "set"},
)


def make_enum_and_map(enum_name, value_map):
    ...
    enum_value = enum.Enum(enum_name, value_map)

    string_map = {s: getattr(enum_value, v) for v, s in value_map.items()}
    return enum_value, string_map


BYTE_ORDER, STRING_ENUM_MAP = make_enum_and_map(
    "BYTE_ORDER", {"BIG_ENDIAN": "bigEndian", "LITTLE_ENDIAN": "littleEndian"}
)
...
TYPE_PRIMITIVE_TYPE, TYPE_PRIMITIVE_TYPE_MAP = make_enum_and_map(
    "TYPE_PRIMITIVE_TYPE", PRIMITIVE_TYPE_LIST
)
